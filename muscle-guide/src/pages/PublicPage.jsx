...
// Hook: esconde a barra de filtros ao rolar para baixo
...
// MuscleGroup NÃO gerencia mais o modal — só chama onOpen
function MuscleGroup({ group, exercises, onOpen }) {
  const [open, setOpen] = useState(true)

  const sorted = [...exercises]
    .sort((a, b) => (b.activation_pct || 0) - (a.activation_pct || 0))
    ...
    <section className="muscle-group fade-in">
      ...
      {open && (
        <div className="group-body fade-in">
          ...
          {sorted.map(ex => (
            <ExerciseRow key={ex.id} exercise={ex} onOpen={onOpen} />
          ))}
        </div>
      )}
    </section>
...
export default function PublicPage() {
  ...
  // Modal gerenciado na RAIZ — fora de qualquer overflow:hidden
  ...
  const filteredGroups = groups.filter(g => !activeGroup || g.id === activeGroup)
  ...
  return (
    <div className="public-page">
      ...
      {/* Barra de filtros — esconde ao rolar */}
      <div className={`controls-bar${controlsHidden ? ' controls-bar--hidden' : ''}`}>
        <div className="container controls-inner">
          ...
          <div className="group-pills">
            <button className={`pill ${!activeGroup ? 'active' : ''}`}
              onClick={() => setActiveGroup(null)}>Todos</button>
            {groups.map(g => (
              <button key={g.id}
        ...
        {filteredGroups.map(g => {
          ...
          return (
            <MuscleGroup
              ...
              group={g}
              exercises={exs}
              onOpen={setSelectedExercise}   // passa o setter para a raiz
            />
          )
        })}
      </main>
      ...
      {/* Modal renderizado NA RAIZ — fora de qualquer overflow:hidden */}
      {selectedExercise && (
        <ExerciseModal
